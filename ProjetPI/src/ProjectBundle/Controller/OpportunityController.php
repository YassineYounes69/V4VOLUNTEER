...
$em->flush();
    ...
    public function getRealEntities($results){
        foreach($results as $results){
            $realEntities[$results->getId()]=[$results->getNomop(), $results->getDomaine(), $results->getId(), $results->getImg1()];
        }
        return $realEntities;
    }
    ...
    public function opSearchAction(Request $request){
        ...
        if($request->isMethod("POST")){
            ...
            $results =  $em->getRepository('ProjectBundle:Opportunity')->findOPS($domaine,$dateDebut,$dateFin);
            if (!$results){
                $result="Aucune opportunité trouvée";
                $nbOp=0;
            }
            else {
                $result=$results;
